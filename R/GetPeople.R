...
ExtractDataByLocation <- function(County_DEDs,
  ...
  if(is.null(Pre_Loaded_Data)){
    ...
    tbl1 = lapply(full_file_paths[which_files], LoadCleanRaw,
...
ExtractDataByLocation_fread <- function(County_DEDs,
                                  path_1901 = "data/census_ireland_1901",
                                  path_1911 = "data/census_ireland_1911",
                                  Pre_Loaded_Data = NULL){
  ...
  if(is.null(Pre_Loaded_Data)){
    ...
    tbl1 = lapply(full_file_paths[which_files], LoadCleanRaw_fread, assignID = c("County", "DED", "Year"))
    # for(i in 1:length(tbl1)){
    #   tbl1[[i]] <- mutate_all(tbl1[[i]], as.character)
    # }
    data_within_radius = tbl1 %>% dplyr::bind_rows()
